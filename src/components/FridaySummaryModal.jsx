...
import { CalendarDays, X, TrendingUp, Trophy } from 'lucide-react';
...
export function FridaySummaryModal({ isOpen, onClose, userName, stats }) {
    ...
    return (
        <div className="fixed inset-0 bg-slate-900/40 backdrop-blur-sm z-50 flex items-center justify-center p-4 animate-in fade-in duration-200">
            <div className="bg-white rounded-2xl w-full max-w-md shadow-xl border border-slate-100 p-6 relative animate-in zoom-in-95 duration-200">
                <button
                    onClick={onClose}
                    className="absolute top-4 right-4 text-slate-400 hover:text-slate-600 transition-colors bg-slate-50 hover:bg-slate-100 rounded-full p-1.5"
                >
                    <X className="w-5 h-5" />
                </button>
